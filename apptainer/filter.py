...
def filter_dir_and_print(args):
    for issue in os.listdir(args.collection_dir):
        ...
        mets = etree.parse(os.path.join(
            args.collection_dir, issue, 'mets', f'{issue}_METS.xml'))
        namespaces = mets.getroot().nsmap
        if None in namespaces:
            # lxml doesn't support the empty namespace
            namespaces['_'] = namespaces[None]
            namespaces.pop(None)
        date_issued = mets.xpath('//MODS:dateIssued',
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--start-date', action='store',
                           type=_date_formatter)
    argparser.add_argument('--end-date', action='store',
                           type=_date_formatter)
