...
class RemoveDeletedBindingsOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        self.log.info(
            f"::group::Removing the following bindings from {self.target_path}"
        )
        ...
        deleted_binding_globs = [
            utils.binding_download_directory(binding_id) + "/*"
        ...
        with ssh_hook.get_conn() as ssh_client:
            cmd = (
                ...
                "module load libzip && "
                'zip -d {self.target_path} {" ".join(deleted_binding_globs)}'
            )
            self.ssh_execute_and_raise(
                ssh_client,
                cmd,
